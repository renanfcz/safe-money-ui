...
function getTotal(expenses: Expense[]) {
  return expenses.reduce((total, expense) => total + expense.value, 0);
}

export function Table({ month, year, expenses }: ExpenseGroupProps) {
  ...
  const categories = useQuery({
    ...
    queryFn: (): Promise<Categories> => {
      return executeQuery(CATEGORIES, null, getToken());
    },
  });
  ...
  return (
    <div className="px-5 w-screen">
      <div className="my-5 rounded shadow bg-white">
